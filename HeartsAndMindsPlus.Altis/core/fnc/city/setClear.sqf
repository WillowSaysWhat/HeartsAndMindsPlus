...
if (_city getVariable ["marker", ""] != "") then {
    (_city getVariable ["marker", ""]) setMarkerColor "colorBLUFOR"; // CONFIG OBJ Colors
};
...
//ADDNOTIF OBJ CLEAR
[parseText "<t color='#FACE00' font='PuristaBold' align = 'right' shadow = '1.5' size='2'>+ Objective Cleared! </t><br /><t  align = 'right' shadow = '1.5' size='1.5'>+$50</t>", [0, 0.5, 1, 1], nil, 5, 1.7, 0] remoteExec ["BIS_fnc_textTiles", 0];
    [west, 50, false] call acex_fortify_fnc_updateBudget; 
    btc_global_economy = btc_global_economy + 50;

if (btc_debug) then {
    private _id = _city getVariable "id";
    (format ["loc_%1", _id]) setMarkerColor "colorBLUFOR"; // CONFIG OBJ Colors
};
